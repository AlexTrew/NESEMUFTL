...
static void write_abs_instruction(CpuState* cpu, GHashTable* opcode_lookup, const  char* instruction, uint16_t operand) {
...
void assemble(CpuState* cpu, const char* filename) {
  ...
  while(fgets(line_buf, MAX_LINE_LEN, file)){
    ...
    if (!regexec(&abs_regex, line_buf, 0, NULL, 0)) {
      ...
      write_abs_instruction(cpu, opcode_lookup, instruction, operand);
    } 
